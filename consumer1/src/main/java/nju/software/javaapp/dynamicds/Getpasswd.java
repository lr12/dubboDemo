...
public class Getpasswd {
	...
	public String passwd(String l_fydm, String l_xlh) {
		...
		for (i = 0; i < lf1; i++) {
			...
			l_return = l_return.substring(0, l_pos - 1)
					+ l_xlh.substring(l_pos - 1, l_pos)
					+ l_return.substring(l_pos - 1);
		}
		...
		for (int i = 0; i < 16; i++) {
			if (l_return.contains("-")) {
				...
				l_return = l_return.substring(0, lr) + "_"
						+ l_return.substring(lr + 1);
			}
		}
	 ...
	 * 
	 * getpasswd oo=new getpasswd(); String l_return=oo.passwd(l_fydm,l_xlh);
	 * 
	 * { System.out.println(l_return); } }
	 */
}
